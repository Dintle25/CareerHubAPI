...
// Application form — uses toast notifications for API success and error responses.
// Field-level validation errors (Zod) stay inline next to each input — they tell
// the user what to fix before submitting. Toasts are for API responses only.
...
const applicationSchema = z
  .object({
    ...
    yearsOfExperience: z.number().int("Must be a whole number").min(0, "Cannot be negative").max(50, "Must be 50 or less"),
    coverLetter: z.string().min(50, "Cover letter must be at least 50 characters — tell us why you're a strong fit").max(2000, "Cover letter must be at most 2000 characters"),
    ...
    noticePeriodWeeks: z.number().int("Must be a whole number").min(0, "Cannot be negative"),
  })
  .refine(
    (data) => data.availableImmediately || data.noticePeriodWeeks > 0,
    { message: "Please enter your notice period in weeks", path: ["noticePeriodWeeks"] }
  );
...
export default function ApplicationForm({ jobId, jobTitle }: ApplicationFormProps) {
  ...
  const { register, handleSubmit, reset, watch, formState: { errors, isSubmitting } } = useForm<ApplicationFormData>({
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      // Success toast — replaces the inline green success banner
      toast.success(`Application for "${jobTitle}" submitted! We'll be in touch soon.`);
      queryClient.invalidateQueries({ queryKey: ["jobs"] });
      reset();
    },
    onError: (error) => {
      // Error toast — replaces the inline red error banner
      // Field errors are NOT shown here — they appear inline next to the field
  ...
  return (
    <div className="w-full max-w-2xl mx-auto">
      ...
      {/* No inline success or error banner here — both are now toasts.
          Field-level Zod errors still appear inline under each input below. */}
      ...
      <form onSubmit={handleSubmit(onValid, (errors) => console.log("VALIDATION ERRORS:", errors))} noValidate className="space-y-5">
        ...
        <div>
          ...
          />
          {/* Inline field error — stays here, does not become a toast */}
        ...
        <div>
          ...
          />
          {errors.yearsOfExperience && <p className="mt-1 text-xs text-red-600 dark:text-red-400">{errors.yearsOfExperience.message}</p>}
        </div>
        ...
        <div>
          ...
          />
          {errors.coverLetter && <p className="mt-1 text-xs text-red-600 dark:text-red-400">{errors.coverLetter.message}</p>}
        </div>
        ...
        <div>
          ...
          />
          {errors.linkedInUrl && <p className="mt-1 text-xs text-red-600 dark:text-red-400">{errors.linkedInUrl.message}</p>}
        </div>
        ...
        {/* Notice Period — only shown when not available immediately */}
        {!availableImmediately && (
          <div>
            ...
            />
            {errors.noticePeriodWeeks && <p className="mt-1 text-xs text-red-600 dark:text-red-400">{errors.noticePeriodWeeks.message}</p>}
          </div>
        )}
